"""
Agente 2 — TikTok Users  (Bronze → Silver)
centinela.tiktok_usuarios → silver.tiktok_usuarios

Clasifica username + nombre + bio en lotes de BATCH_SIZE.
...
bronze_col = client["centinela"]["tiktok_usuarios"]
silver_col = client["silver"]["tiktok_usuarios"]
...
_RE_URL      = re.compile(r"https?://|www\.|\.com|\.mx|\.io|linktr\.ee", re.I)
...
def _texto_perfil(doc: dict) -> str:
    partes = [str(doc.get(c, "")).strip()
              for c in ("username", "nombre", "bio") if doc.get(c)]
...
def _build_op(doc: dict, texto: str, top_label: str,
    ...
    silver.update({
        ...
        "tiene_bio":             bool(bio),
        "longitud_bio":          len(bio),
        "contiene_url_bio":      bool(_RE_URL.search(bio)),
        "contiene_telefono_bio": bool(_RE_TELEFONO.search(bio)),
        "fuente":                "tiktok",
        "coleccion_origen":      "centinela.tiktok_usuarios",
...
def ejecutar_filtro_tiktok_users(clf=None):
    ...
    if total == 0:
        print("  TikTok Users  ·  sin registros nuevos")
        return {"agente": "agente2_tiktok_users", "sospechosos": 0, "total": 0}
    ...
    def _procesar_buffer():
        ...
        for doc, texto, res in zip(buf_docs, buf_textos, resultados):
            ...
            sospechosos += 1
            user = f"@{doc.get('username', doc['_id'])}"[:22]
    ...
    print(f"\n  TikTok Users  ·  {sospechosos}/{total} sospechosos  ·  tasa={tasa}")
    return {"agente": "agente2_tiktok_users", "sospechosos": sospechosos, "total": total}
